        ...
        {if !$opc}
          ...
          <div id="opc_payment_methods" class="opc-main-block">
            ...
            <div class="paiement_block">
              ...
              <div id="order-detail-content" class="table_block">
                <table class="data-table cart-table" id="shopping-cart-table">
                  ...
                  <thead class="hidden-phone">
                    <tr class="first last">
                      <th rowspan="1"><span class="nobr">{l s='Product'}</span></th>
                      <th colspan="1" class="a-center"><span class="nobr">{l s='Description'}</span></th>
                      <th class="a-center" rowspan="1">{l s='Avail.'}</th>
                      <th colspan="1" class="a-center">{l s='Unit price'}</th>
                      <th colspan="1" class="a-center">{l s='Qty'}</th>
                      <th colspan="1" class="a-center">{l s='Total'}</th>
                    </tr>
                  </thead>
                  <tfoot>
                  ...
                  <tr class="cart_total_voucher" {if $total_discounts == 0}style="display: none;"{/if}>
                    <td colspan="5"> {if $use_taxes}
                      ...
                      {/if} </td>
                    <td class="price-discount price" id="total_discount"> {if $use_taxes}
                  ...
                  <tr class="cart_total_voucher" {if $total_wrapping == 0}style="display: none;"{/if}>
                    <td colspan="5"> {if $use_taxes}
                      ...
                      {/if} </td>
                    <td class="price-discount price" id="total_wrapping"> {if $use_taxes}
                  ...
                  {if $priceDisplay}
                    <tr class="cart_total_delivery" {if $shippingCost 
                  ...
                  {if $use_taxes}
                  <tr class="cart_total_price">
                    <td colspan="5" id="cart_voucher" class="cart_voucher"> {if $voucherAllowed}
                      ...
                      {/if} </td>
                    <td colspan="1" class="price total_price_container" id="total_price_container"><p>{l s='Total:'}</p>
                      <span>{displayPrice price=$total_price}</span></td>
                  </tr>
                  {else}
                  <tr class="cart_total_price">
                    <td colspan="5" id="cart_voucher" class="cart_voucher"> {if $voucherAllowed}
                      <div id="cart_voucher" class="table_block"> {if isset($errors_discount) && $errors_discount}
                        <ul class="error">
                          {foreach from=$errors_discount key=k item=error}
                          <li>{$error|escape:'htmlall':'UTF-8'}</li>
                          {/foreach}
                        ...
                        <form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
                          <fieldset>
                            ...
                            <p class="submit">
                              ...
                              <input type="submit" name="submitAddDiscount" value="{l s='ok'}" class="button" />
                            </p>
                            {if $displayVouchers}
                            <h4 class="title_offers">{l s='Take advantage of our offers:'}</h4>
                            <div id="display_cart_vouchers"> {foreach from=$displayVouchers item=voucher} <span onclick="$('#discount_name').val('{$voucher.name}');return false;" class="voucher_name">{$voucher.name}</span> - {$voucher.description} <br />
                              {/foreach} </div>
                            {/if}
                          </fieldset>
                        </form>
                      ...
                      {/if} </td>
                    <td colspan="1" class="price total_price_container" id="total_price_container"><p>{l s='Total:'}</p>
                      <span id="total_price">{displayPrice price=$total_price_without_tax}</span></td>
                  </tr>
                  {/if}
                    </tfoot>
                  ...
                  <tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" class="alternate_item cart_item">
                    <td colspan="4"> {foreach from=$customization.datas key='type' item='datas'}
                      ...
                      {/foreach} </td>
                    <td class="cart_quantity"> {if isset($cannotModify) AND $cannotModify == 1} <span style="float:left">{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.cart_quantity-$quantityDisplayed}{/if}</span> {else}
                      <div style="float:right"> <a rel="nofollow" class="cart_quantity_delete" id="{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" href="{$link->getPageLink('cart', true, NULL, "delete&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$token_cart}")}"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete'}" title="{l s='Delete this customization'}" width="11" height="13" class="icon" /></a> </div>
                      <div id="cart_quantity_button" style="float:left"> <a rel="nofollow" class="cart_quantity_up" id="cart_quantity_up_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" href="{$link->getPageLink('cart', true, NULL, "add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$token_cart}")}" title="{l s='Add'}"><img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Add'}" width="14" height="9" /></a><br />
                  ...
                  <tr class="cart_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
                    ...
                    <td class="cart_discount_price"><span class="price-discount"> {if $discount.value_real > 0}
              ...
              {if $opc}
              <div id="opc_payment_methods-content">{/if}
              ...
              {if !$opc}
              <p class="cart_navigation sitemapbttom"><a href="{$link->getPageLink('order', true, NULL, "step=1")}" title="{l s='Previous'}" class="button"><span><span>&laquo; {l s='Previous'}</span></span></a></p>
              {else} </div>
            {/if} </div>
